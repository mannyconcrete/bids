...
class Database:
    ...
    def get_projects(self):
        """Get all projects"""
        try:
            self.cursor.execute("SELECT name, owner FROM projects")
    ...
    def get_project_owner(self, project_name):
        """Get project owner"""
        try:
            self.cursor.execute("SELECT owner FROM projects WHERE name = ?", (project_name,))
            result = self.cursor.fetchone()
            return result[0] if result else None
        except Exception as e:
            print(f"Error getting project owner: {str(e)}")
            return None
