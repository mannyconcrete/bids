...
def get_project_sheet_name(project_name):
    ...
    valid_name = f"Locations_{valid_name.replace(' ', '_')[:30]}"
    return valid_name

def clean_project_name(project_name):
    """Remove location information from project name"""
    # Split on common location separators and take the first part
    separators = [' - ', ' – ', ' — ', ' at ', ' in ', ' near ']
    cleaned_name = project_name
    for separator in separators:
        cleaned_name = cleaned_name.split(separator)[0]
...
def project_status_dashboard(spreadsheet):
    ...
    # Clean project names for display
    project_names = [clean_project_name(p[0]) for p in projects]
    
    # Project selection with cleaned names
    selected_project = st.selectbox("Select Project", project_names)
    
    if selected_project:
        # Get the original project name with location
        original_project = next(p[0] for p in projects if clean_project_name(p[0]) == selected_project)
        project_owner = db.get_project_owner(original_project)
        st.info(f"Project Owner: {project_owner}")
        
        # Use cleaned name for sheet
