...
def ensure_project_sheet_exists(spreadsheet, project_name):
    ...
    try:
        ...
        sheet = spreadsheet.add_worksheet(sheet_name, 1000, 7)
        # Add headers
        headers = [
            "Address",
            "Status",
        ...
        if selected_project:
            ...
            if add_location and new_location:
                try:
                    ...
                    if not locations_df.empty and new_location in locations_df['Address'].values:
                    ...
                    if geo_location:
                        # Add to project's location sheet
                        new_row = [
                            new_location,
                            new_status,
                            json.dumps([geo_location.latitude, geo_location.longitude]),
                            '',  # notes
                            json.dumps({stage: False for stage in CONCRETE_CHECKLIST}),
            ...
            if not locations_df.empty:
                    ...
                    for _, location in locations_df.iterrows():
                        try:
                            ...
                            <div style='width: 200px'>
                                ...
                                <p><b>Status:</b> {stage_info['icon']} {location['Status']}</p>
                                <p><b>Notes:</b> {location.get('Notes', 'N/A')}</p>
                                <p><b>Added:</b> {location['DateAdded']}</p>
                            </div>
                            """
                ...
                for idx, location in locations_df.iterrows():
                    with st.expander(f"{PROJECT_STAGES[location['Status']]['icon']} {location['Address']}"):
                        ...
                        with col1:
                            new_status = st.selectbox(
                                ...
                                list(PROJECT_STAGES.keys()),
                                key=f"status_{idx}",
                                index=list(PROJECT_STAGES.keys()).index(location['Status'])
                            )
                            
                            if new_status != location['Status']:
                                # Update status in spreadsheet
                                row_num = idx + 2  # +2 for header and 0-based index
                                locations_sheet.update_cell(row_num, 2, new_status)
                                st.rerun()
                        
                        with col2:
                            checklist = json.loads(location['Checklist'])
                            ...
                            for stage, info in CONCRETE_CHECKLIST.items():
                                checked = st.checkbox(
                                    f"{info['icon']} {stage}",
                                    value=checklist.get(stage, False),
                                    key=f"check_{idx}_{stage}"
                                )
                                if checked != checklist.get(stage, False):
                                    checklist[stage] = checked
                                    checklist_updated = True
                            
                            if checklist_updated:
                                row_num = idx + 2
                                locations_sheet.update_cell(row_num, 5, json.dumps(checklist))
                                st.rerun()
                        ...
                        if new_notes != location.get('Notes', ''):
                            row_num = idx + 2
                            locations_sheet.update_cell(row_num, 4, new_notes)
                            st.rerun()
                        
                        # Delete button
                        if st.button("Delete Location", key=f"delete_{idx}"):
                            row_num = idx + 2
                            locations_sheet.delete_row(row_num)
                            st.rerun()
            else:
                st.info("No locations added yet for this project")
                
    except Exception as e:
        st.error(f"Error managing project locations: {str(e)}")
